...
| 항목 | array[다음 중 하나: link:#managedCluster_1_2_get_response_body["managedCluster_1.2_get_response_body"], array[]] | 참  a| 
...
|===
[[managedCluster_1_2_get_response_body]]
[role="api-collapsible-fifth-title"]
...
| 이름 | 문자열 | 참  a| 
클러스터 이름을 포함하는 JSON 문자열입니다. 정의된 값은 다음과 같습니다.
...
* "unknown" - 클러스터의 상태를 확인할 수 없습니다




| restoreTargetSupported | 문자열 | 거짓  a| 
관리형 클러스터가 애플리케이션 데이터 관리 작업의 _대상_이 될 수 있는지 여부를 나타내는 값을 포함하는 JSON 문자열입니다. 정의된 값은 다음과 같습니다.
...
관리형 클러스터가 애플리케이션 데이터 관리 백업과 유사한 작업을 수행할 수 있는지 여부를 나타내는 값을 포함하는 JSON 문자열입니다. 정의된 값은 다음과 같습니다.

* "true" - 클러스터는 앱 데이터 관리 작업을 수행할 수 있습니다.
...
클러스터가 "실행 중" 상태로 전환되지 못하는 이유, "실행 중" 상태에서 문제가 발생하는 이유, 또는 "실패"나 "제거됨" 태로 전환된 이유를 각각 나타내는 JSON 문자열의 JSON 배열입니다. 정의된 값은 다음과 같습니다.
...
* "ineligible" - 클러스터가 "running" 상태이며 관리 대상이 아니고 관리 대상 자격도 없습니다. 이러한 관리 대상 아님의 이유는 unready 키 값에 제공됩니다.
...
* "partial" - 클러스터에는 스냅샷을 지원하지 않는 스토리지 클래스만 있습니다.
...
클러스터 관리가 불가능한 이유를 각각 나타내는 JSON 문자열들의 JSON 배열입니다. 정의된 값은 다음과 같습니다.
...
"state"가 "managed" 또는 "unmanaged"로 전환된 이유에 대한 추가 세부 정보를 나열하는 JSON 객체의 JSON 배열입니다. + 사용 가능한 세부 정보가 없는 경우 이 배열은 비어 있습니다.
...
| inUse | 문자열 | 참  a| 
클러스터에 관리형 애플리케이션이 있는지 여부를 나타내는 값을 포함하는 JSON 문자열이므로 삭제할 수 없습니다. 정의된 값은 다음과 같습니다.
...
| clusterVersion | 문자열 | 참  a| 
클러스터 환경의 시맨틱 버전 표현을 포함하는 JSON 문자열입니다. Kubernetes 클러스터의 경우 Kubernetes 버전입니다. 정의된 값은 다음과 같습니다.
...
* Astra 식별자 스키마를 준수합니다. 포함되지 않은 경우 아직 검색되지 않은 것입니다.
...
비공개 클러스터의 locationID가 포함된 JSON 문자열입니다.
